        ...
        Right (ms', _) ->
          case desugar ms' of
            ...
            Right modules -> do
              ...
              mkdirp contentsFile
              TL.writeFile contentsFile (H.renderHtml $ contentsPageHtml outputDir modules)
...
renderModule outputDir bookmarks m@(P.Module moduleName _ exps) = do
  let filename = outputDir </> filePathFor moduleName
      html = H.renderHtml $ moduleToHtml outputDir bookmarks m
  mkdirp filename
...
moduleToHtml :: FilePath -> [(P.ModuleName, String)] -> P.Module -> H.Html
moduleToHtml outputDir bookmarks (P.Module moduleName ds exps) = 
